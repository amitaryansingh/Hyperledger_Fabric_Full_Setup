...
import { Separator } from "@/components/ui/separator";
import { toast } from "sonner";
import { PlusCircle, Trash2, LandPlot, Users, FilePlus } from 'lucide-react';
...
export function LandParcelForm({ onFormSubmit }: LandParcelFormProps) {
  const [formData, setFormData] = useState<LandParcelData>({
    ...
    tehsil: "",
    area: 0,
    owners: [{ name: "", aadhaar: "", bankInfo: "", share: 100 }],
  });
  const [isLoading, setIsLoading] = useState(false);
  ...
  const handleOwnerChange = (index: number, e: React.ChangeEvent<HTMLInputElement>) => {
    const { name, value, type } = e.target;
    const newOwners = [...formData.owners];
    newOwners[index] = { ...newOwners[index], [name]: type === 'number' ? parseFloat(value) || 0 : value };
    setFormData((prev) => ({ ...prev, owners: newOwners }));
  };
  ...
  const addOwner = () => {
    setFormData((prev) => ({
      ...prev,
      owners: [...prev.owners, { name: "", aadhaar: "", bankInfo: "", share: 0 }],
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const totalShare = formData.owners.reduce((acc, owner) => acc + owner.share, 0);
    if (totalShare !== 100) {
      toast.error("Validation Error", {
        description: "The sum of all owner shares must be exactly 100%.",
        duration: 5000,
      });
      setIsLoading(false);
      return;
    ...
    try {
      const response = await axios.post("http://localhost:8080/api/parcels", payload);
      toast.success("Success!", {
        description: `Land Parcel ${response.data.surveyNumber} created and logged to the blockchain.`,
        duration: 5000,
      });
      setFormData({
        surveyNumber: "", village: "", tehsil: "", area: 0,
        owners: [{ name: "", aadhaar: "", bankInfo: "", share: 100 }],
      });
      onFormSubmit();
    } catch (error: any) {
  ...
  return (
    <Card className="w-full max-w-5xl mx-auto border-2 shadow-lg">
      <CardHeader>
        ...
        <CardDescription>
          Step 1: Digitize the basic details of a new land parcel and its owners.
        </CardDescription>
      </CardHeader>
      <form onSubmit={handleSubmit}>
        <CardContent className="space-y-6 pt-6">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-4 border rounded-lg bg-slate-50/50">
              ...
              <div>
                <Label htmlFor="tehsil">Tehsil</Label>
                <Select name="tehsil" onValueChange={(value) => handleSelectChange('tehsil', value)} value={formData.tehsil} required>
                  <SelectTrigger><SelectValue placeholder="Select Tehsil" /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="Wardha">Wardha</SelectItem>
                    <SelectItem value="Deoli">Deoli</SelectItem>
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-semibold flex items-center gap-2"><Users className="h-5 w-5" /> Owner Details</h3>
            {formData.owners.map((owner, index) => (
              <div key={index} className="flex flex-col md:flex-row items-start gap-4 p-4 border rounded-lg bg-slate-50/50">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 flex-grow">
                  ...
                  <div>
                   ...
                   <div>
                    <Label htmlFor={`owner-share-${index}`}>Share (%)</Label>
                    <Input id={`owner-share-${index}`} name="share" type="number" step="0.01" placeholder="e.g., 50.0" value={owner.share} onChange={(e) => handleOwnerChange(index, e)} required />
                  </div>
                  <div className="md:col-span-4">
                    <Label htmlFor={`owner-bank-${index}`}>Bank Info</Label>
                    <Input id={`owner-bank-${index}`} name="bankInfo" placeholder="e.g., SBI - 1234567890" value={owner.bankInfo} onChange={(e) => handleOwnerChange(index, e)} required />
                  </div>
                </div>
                {formData.owners.length > 1 && (
                  <Button type="button" variant="ghost" size="icon" onClick={() => removeOwner(index)} className="mt-6 md:mt-5 text-red-500 hover:bg-red-100 hover:text-red-600">
                    <Trash2 className="h-4 w-4" />
                  </Button>
                )}
              </div>
