...
//
//@RestController
//@RequestMapping("/api/parcels") // Base URL for all endpoints in this controller
...
//
//    /**
//     * Endpoint to create a new land parcel.
//     * Handles POST requests to /api/parcels
//     *
//     * @param landParcel The land parcel data from the request body.
//     * @return The created land parcel with its database ID, or an error message.
//     */
//    @PostMapping
//    public ResponseEntity<?> createLandParcel(@RequestBody LandParcel landParcel) {
//        try {
//            LandParcel createdParcel = landAcquisitionService.createLandParcel(landParcel);
//            // Return the created object with an HTTP 201 Created status
//            return new ResponseEntity<>(createdParcel, HttpStatus.CREATED);
//        } catch (Exception e) {
//            // Return a detailed error message with an HTTP 500 Internal Server Error status
//            return new ResponseEntity<>("Error creating land parcel: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
//        }
//    }
//
//    // We will add more endpoints here later for GET, PUT, etc.
//
//    @GetMapping
//    public ResponseEntity<List<LandParcel>> getAllLandParcels() {
//        List<LandParcel> parcels = landAcquisitionService.getAllParcels();
